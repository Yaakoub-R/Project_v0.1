...
import { deleteProfile, deleteUser, updateProfile, updateUser } from "../Redux/Actions/authActions"
...
const UserCardB=({profile})=>{
  ...
  const handleEdit=(e)=>{
    e.preventDefault()
    dispatch(updateUser(profile._id,{name,lastName,email},navigate))}
  ...
  return (
   ...
   Edit Profile
        ...
        <Button  onClick={(e)=>{dispatch(deleteUser(profile._id,navigate))}} rightIcon={<WarningTwoIcon />} colorScheme='teal' variant='outline'>
    Delete Profile
  </Button>
  </Stack>
  <Modal
